...
static bool btmtk_is_bin_file_mode(uint8_t *buf, size_t buf_size)
{
	...
	if (*ptr == AUTO_MODE) {
		BTMTK_WARN("It's Auto mode: %c", *ptr);
		ret = btmtk_efuse_read(addr, &value);
		if (ret) {
			BTMTK_WARN("btmtk_efuse_read fail");
		...
		BTMTK_WARN("It's not Auto/EEPROM mode %c", *ptr);
		ret = false;
	}
					...
					const struct sdio_device_id *id)
{
	...
	pf_sdio_reset = (sdio_reset_func) kallsyms_lookup_name("sdio_reset_comm");
